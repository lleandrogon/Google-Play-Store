...
display(apps.limit(5))

# COMMAND ----------

display(user_reviews.limit(5))

# COMMAND ----------

display(apps.join(user_reviews, on = "app", how = "inner"))

# COMMAND ----------

(apps.join(user_reviews, on = "app", how = "inner")).printSchema()

# COMMAND ----------

display(
    apps.join(user_reviews, on = "app", how = "inner") \
    .groupBy("app") \
    .agg(
...
df.write.mode("overwrite") \
    .format("delta") \
    .option("mergeSchema", "true") \
    .saveAsTable("google.play_store.gold_app_polarity_subjectivity")